            ...
            {parkings.map((parking) => (
              <div key={parking.id} className="border border-gray-200 rounded-lg p-4">
                ...
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="font-medium">{parking.current_occupancy}/{parking.capacity}</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2 mb-4">
                  <div 
                    className="h-2 rounded-full bg-blue-500 transition-all"
                    style={{ 
                      width: `${(parking.current_occupancy / parking.capacity) * 100}%`,
                      backgroundColor: (parking.current_occupancy / parking.capacity) > 0.8 ? '#ef4444' : 
                                    (parking.current_occupancy / parking.capacity) > 0.6 ? '#f59e0b' : '#10b981'
                    }}
                  ></div>
                </div>
